...
export function useGameLoop(
  ...
  const [citizens, setCitizens] = useState<
    { x: number; y: number; id: string; vaccinated: boolean }[]
  >(
    phase.hasVIPs
      ? [
          { x: 2, y: 1, id: "c1", vaccinated: false },
          { x: 5, y: 3, id: "c2", vaccinated: false },
          { x: 8, y: 2, id: "c3", vaccinated: false },
          { x: 1, y: 6, id: "c4", vaccinated: false },
          { x: 6, y: 7, id: "c5", vaccinated: false },
          { x: 9, y: 6, id: "c6", vaccinated: false },
        ]
      : [],
  );
  ...
  useEffect(() => {
    vaccineReadyRef.current = vaccineReady;
  }, [vaccineReady]);
  useEffect(() => {
  ...
  const spawnWave = useCallback(
    (index: number) => {
      ...
      queue.push(...queue.splice(0).sort((a, b) => a.time - b.time));
      waveState.current = {
        queue,
  ...
  const loop = useCallback(
    (time: number) => {
        ...
        if (waveState.current.timeToNext <= 0) {
          ...
          if (waveState.current.queue.length > 0) {
            waveState.current.timeToNext =
              waveState.current.queue[0].time - toSpawn.time;
          }
        }
      }

      // ── Wave done auto-start ────────────────────────────────────────────────
      if (waveRunningRef.current && waveState.current.queue.length === 0) {
      ...
      // ── Torres: Ataque e Combate Corpo a Corpo ───────────────────────────
        ...
        const ts = prevTowers.map((tower) => {
          ...
          if (tower.lastAttack <= 0) {
            ...
            if (target) {
              let speedMult = 1;
              if (
                tower.def.type === "memoria" &&
                vaccinesActiveRef.current.includes(target.def.type)
              ) {
                speedMult = 3;
              }
